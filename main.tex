...
Since the gradient of softmax is known, we can directly verify that the IFT gives
the correct answer.
...
which agrees with simply taking the derivative of softmax \citep{sparsemax}.
...
\section{OptNet}
\citet{optnet} apply the IFT to quadratic problems in particular.
The methodology is identical: given a quadratic program (QP),
use the KKT conditions to produce a system of equations then apply the IFT
/ implicit differentiation to compute the derivative of the solution wrt parameters.
This allows 
