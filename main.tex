...
and informally refer to $x$ as a solution, and $\theta$ as parameters.
...
\frac{dF(\theta,x)}{d\theta} = \begin{bmatrix}
    ...
    \vdots & \ddots & \vdots \\
    \frac{dF_m(\theta,x)}{d\theta_m} & \cdots & \frac{dF_m(\theta,x)}{d\theta_n} \\
...
at the solution point $(\theta, z,u,v)$.
...
\label{eqn:qp}
\begin{aligned}
\textrm{minimize } \quad & \frac12 x^\top Q x + q^\top x\\
...
As the application of the IFT is almost identical, we will not cover it in as much
...
\diag(u)(Gz-h) &= \mathbf{0}_n\\
\diag(v)(Az-b) &= \mathbf{0}_{n}.
\end{aligned}
\end{equation}
