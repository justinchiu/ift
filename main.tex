...
\frac{dF(\theta,x)}{d(\theta,x)} = \begin{bmatrix}
    \frac{dF}{d\theta} &\frac{dF}{dx} 
\end{bmatrix},
\end{equation*}
where we have the matrix of partial derivatives
\begin{equation*}
\frac{dF}{d\theta} = \begin{bmatrix}
    \frac{dF_1}{d\theta_1} & \cdots & \frac{dF_1}{d\theta_n} \\
    \vdots & \ddots & \vdots \\
    \frac{dF_m}{d\theta_1} & \cdots & \frac{dF_m}{d\theta_n} \\
\end{bmatrix},
\end{equation*}
and similarly for $\frac{dF}{dx}\in\R^{m\times m}$.

In order to embed an optimization layer inside a neural network,
we must provide the Jacobian for our layer for use in an AD system.\footnote{
