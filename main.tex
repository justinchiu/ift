...
Rather than directly applying the formula for the
derivative, we can also use implicit differentiation to compute $\frac{dx^*}{d\theta}$.
...
defined as the output of an optimization problem, and derive derivatives using the IFT.
...
(Say check output and gradient)
We will start by reviewing softmax and its expression as an optimization problem,
checking the conditions of the IFT hold, then compute gradients.
Since the gradient of softmax is known, we can easily verify that the IFT gives
...
as well \citep{gao2018properties}.
...
and $F \in \mcC^1$.
We must then check that the Jacobian matrix $\frac{dF}{dx}$ is non-singular.
...
\end{bmatrix}
\end{equation}
Since the solution $x^*$ must be feasible, we know that $x^\top\mathbf{1} = 1$ and $u > 0$.
With the additional information that the domain of $H(x)$
adds the implicit constraint that $\forall i, x_i > 0$,
we can deduce that the Jacobian is full rank and therefore has nonzero determinant.
...
we can proceed to compute the Jacobian of softmax by applying the second part of the IFT.

to the version of softmax expressed as an optimization problem in Eqn.~\ref{eqn:softmax-opt}.
In particular, we are interested in the derivative $\frac{dx}{d\theta}$,
the derivative of the output of softmax (obtained by solving the optimization problem)
wrt the parameters $\theta$.

We can solve the system of equations in \ref{eqn:system}
for the entries of the upper-left block corresponding to $\frac{dx}{d\theta}$,
i.e. 
\begin{equation}
\label{eqn:dsystem-matrix}
\begin{bmatrix}
...
we compute the block-wise inverse of
\begin{equation*}
\begin{bmatrix}
A & B\\
C & D
...
\end{bmatrix}
\end{equation*}
We first compute the inverses: $D^{-1} = \diag(1/x)$, and for $A^{-1}$, we use a similar
block-wise inversion formula.
...
\end{equation}
Since $x$ is feasible, we have $A/E = u$.
Then, we have
\begin{equation}
A^{-1} = \begin{bmatrix}
-\diag(x)^{-1} & \mathbf{1}\\
...
With this, we can finally compute the derivative
