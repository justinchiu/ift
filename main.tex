...
As an example, let's say we would like to maximize $\ell = f_3(f_2(f_1(\theta)))$ with a gradient-based method,
where $\ell\in\R$; $f_3:\R^n\to\R$; $f_1,f_2:\R^n\to\R^n$; and $\theta\in\R^n$.
We can compute the derivatives of $\ell$ wrt each layer via the chain rule.
Focusing on the first layer $f_1$,
we have $\frac{d\ell}{df_1} = \frac{db}{df_3}\frac{df_3}{df_2}\frac{df_2}{df_1}$,
