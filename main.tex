...
{\color{red} still rough, ideally incorporate softmax}
{\color{red} refer readers to better tutorials, like \citet{griewank2008autodiff}}
As an example, we would like to maximize $\ell = f_2(f_1(a))$ with a gradient-based method,
where $\ell\in\R$, $f_2:\R^n\to\R$, $f_1:\R^n\to\R^n$, and $a\in\R^n$.
Each $f_i$ is a layer in a neural network, with associated parameters $\theta_i$.
We can compute the derivatives of $\ell$ wrt the parameters of each layer via the chain rule,
i.e. we have $\frac{d\ell}{d\theta_1} = \frac{d\ell}{df_2}\frac{df_2}{df_1}\frac{df_1}{da}$,
...
$$\frac{df_1}{da} = \begin{bmatrix}
\frac{\partial [f_1]_1}{\partial [a]_1} & 
    \frac{\partial [f_1]_1}{\partial [a]_2} &
    \cdots\\
\frac{\partial [f_1]_2}{\partial [a]_1} & 
    \frac{\partial [a]_2}{\partial [a]_2} &
    \cdots \\
\vdots & \vdots & \ddots
\end{bmatrix},
$$
...
using $\frac{d\ell}{df_i}\frac{df_i}{d\theta_i}$,
...
layer of focus.
Given this learning setup, we can now proceed to a review of softmax.
...
For a thorough introduction to the KKT conditions, see chapter 5 of \citet{bv-cvxbook}.\footnote{
If this is your first time seeing the KKT conditions, please check out chapter 5 of \citet{bv-cvxbook}
...
\begin{equation}
\begin{aligned}
\nabla_x \mcL(\theta, z,u,v) &= \mathbf{0}_n && \textrm{(stationarity)}\\
...
All that remains is to check that the Jacobian matrix $\frac{dF}{d(z,u,v)}$ is non-singular.

The Jacobian $\frac{dF}{d(z,u,v)}\in\R^{2n+1\times 2n+1}$ is given by
\begin{equation}
\label{eqn:f-jac}
\frac{dF}{d(z,u,v)}=
\begin{bmatrix}
...
We saw something similar with the unit circle example, where the derivative was undefined
for $x=0$ using a particular parameterization.
While the issue in the unit circle case could be solved by using another parameterization
due to symmetry,
there is less symmetry in the softmax problem. {\color{red} check this with an expert}
}
...
Recall that we have the solution $x = (z,u,v)$; we will use $x$ for brevity.
...
\left[\frac{dF}{dx}\right]^{-1} = \begin{bmatrix}
\diag(z)^{-1} & -\mathbf{1}_n & -I_{n\times n} \\
...
feel free to skip ahead to Sec.~\ref{sec:softmax-end} for the conclusion
of the softmax exercise.
...
in $\frac{d^2H(z)}{d[z]_i^2} = \frac{1}{[z]_i}$.
}
