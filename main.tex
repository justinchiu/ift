...
\node at ($2*({sqrt(2) / 2}, {sqrt(2) / 2})$) (x)[below left] {$x_1$};
...
\node at ($2*({sqrt(2) / 2 + .21}, {sqrt(2) / 2 + .21})$) {$\frac{df_1(x_1)}{dx}$};
\draw[thick] ($2*({sqrt(2) / 2 + -.5}, {sqrt(2) / 2 + .5})$) --
    ($2*({sqrt(2) / 2 + .5}, {sqrt(2) / 2 -.5})$);

% singular point
\node at (2,0) (x2)[left] {$x_2$};
...
We now pivot to applying the IFT to variational inference.
