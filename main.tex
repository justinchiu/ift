...
Naively, if the Jacobian of the local system is dense, this comes at the cost of
$O(n^2)$ space and $O(n^3)$ time,
trading off the dependence on $K$ for a larger dependence on $n$.\footnote{
The $O(n^2)$ space comes from storing a dense $n\times n$ Jacobian
(or a factorization thereof),
and the $O(n^3)$ time comes from solving the associated dense linear system.
This will become clear when we cover the IFT.
This can be reduced by using approximate inverse-Jacobian or inverse-Hessian products,
...
using the local parameterization (or solution mapping) $f_1(\theta)=\sqrt{1-\theta^2}$.
...
However, we can use local parameterizations: $f_1(\theta) = \sqrt{1-\theta^2}$ on the upper semicircle ($x>0$)
or $f_2(\theta) = -\sqrt{1-\theta^2}$ on the lower semicircle ($x<0$).
...
\begin{equation}
\begin{aligned}
\frac{d}{dz} \mcL(\theta, z,u,v) &= \mathbf{0}_n && \textrm{(stationarity)}\\
...
\mathbf{1}_n^\top & 0 & \mathbf{0}^\top_n\\
\diag(v) & \mathbf{0}_n & \diag(z)
\end{bmatrix}.
\end{equation}
Since a solution must be feasible, we know that $z^\top\mathbf{1}_n=1$.
...
Strict positivity and complementary slackness imply $v=\mathbf{0}_n$,
so the Jacobian reduces to a block upper-triangular matrix with lower-right block $\diag(z)$
and upper-left block
\[
A = \begin{bmatrix}
...
\end{bmatrix}.
\]
Since $\diag(z)$ is invertible and the Schur complement of $A$ with respect to
$-\diag(z)^{-1}$ is
\[
0 - \mathbf{1}_n^\top \left(-\diag(z)\right)\mathbf{1}_n
= z^\top \mathbf{1}_n = 1,
\]
the block $A$ is invertible as well.
Therefore, the Jacobian of $F$ is full rank and equivalently has nonzero determinant.
...
where
\begin{align*}
A = \begin{bmatrix} -\diag(z)^{-1} & \mathbf{1}_n \\ \mathbf{1}_n^\top & 0 \end{bmatrix}&\qquad\qquad
B = \begin{bmatrix}I_{n\times n} \\ 0\end{bmatrix}\\
C = \begin{bmatrix}\diag(v) & 0\end{bmatrix} &\qquad\qquad
D = \diag(z).
\end{align*}
However, because $z\succ 0$, complementary slackness gives $v = 0$,\footnote{
We apply the IFT to solutions where $z\succ 0$ due to a divide-by-zero issue
in $\frac{\partial^2 H(z)}{\partial z_i^2} = -\frac{1}{z_i}$ for $H(z) = -\sum_j z_j \log z_j$.
}
...
Let us denote
$$A
= \begin{bmatrix} -\diag(z)^{-1} & \mathbf{1}_n \\ \mathbf{1}_n^\top & 0 \end{bmatrix}
= \begin{bmatrix}E & F \\ G & H\end{bmatrix}.
$$
First, we compute the Schur complement of $A$,
\begin{equation}
A/E = H - GE^{-1}F = 0 - \mathbf{1}_n^\top(-\diag(z))\mathbf{1}_n = z^\top\mathbf{1}_n.
\end{equation}
Since $z$ is feasible, we have $A/E = 1$ due to the equality constraints
...
A^{-1} 
&= \begin{bmatrix}
    -\diag(z) + \diag(z)\mathbf{1}_n\mathbf{1}^\top_n\diag(z)
    & \diag(z)\mathbf{1}_n\\
    \mathbf{1}^\top_n \diag(z) & 1
\end{bmatrix}\\
&= \begin{bmatrix}
    -\diag(z) + zz^\top
    & z\\
...
Without sparsity, solving the linear system defined by the Jacobian of $F$,
$\frac{dF(\theta,x)}{dx}$, costs $O(n^3)$ time and $O(n^2)$ space in this
softmax example, since $x=(z,u,v)$ has dimension $2n+1=O(n)$.
By contrast, computing softmax itself or applying its Jacobian to a vector costs $O(n)$,
while explicitly materializing the full Jacobian
$\diag(z) - zz^\top$ costs $O(n^2)$ time and $O(n^2)$ space.
...
where unrolling $K$ solver steps and applying reverse-mode AD would take
at least $\Omega(Kn)$ space when the iterates are $n$-dimensional,
while naive implicit differentiation would instead store the dense Jacobian of the
local system, taking $O(d^2)$ space for a $d$-dimensional system.
...
$\frac{d}{dz}\mcL = 0$
already involved derivatives of the Lagrangian $\mcL$ with respect to the primal variables.
In general, this means that in order to apply the IFT to solutions of optimization problems,
we must solve a linear system involving the Jacobian of the KKT conditions.\footnote{
From the IFT, we can either work with the full inverse of this Jacobian
or only the corresponding inverse-matrix-vector product,
which can sometimes be cheaply approximated.
In unconstrained problems this reduces to working with an inverse Hessian,
while constrained problems lead to a larger KKT system that also includes dual variables.
}
If this Jacobian/KKT matrix is dense and has dimension $d$,
storing it costs $O(d^2)$ space
and solving the corresponding linear system costs $O(d^3)$ time.\footnote{
...
such as with approximate inverse-matrix-vector-product techniques
...
For softmax, let us pretend we solved the softmax problem with an iterative method
in $K$ steps, each costing $O(n)$ time.
Computing the derivative from the execution trace would then take $O(Kn)$ time
using reverse-mode AD,
while the naive IFT would take $O(n^3)$ time because the KKT system has size $2n+1$.
Under this simplified model, implicit differentiation is asymptotically faster
only when $K > n^2$.
...
the only nontrivial part is checking that the Jacobian matrix $\frac{dF(\theta,x)}{dx}$ is
