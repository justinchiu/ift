...
guarantees the existence of the implicit solution mapping $x^*(\theta) = x_K$
(dashed line)
...
The nomenclature vector-Jacobian-product used widely in backpropagation
comes from left-to-right processing of the chain rule $\frac{d\ell}{df}\frac{df}{dg}\cdots$.
...
we must compute the Jacobian wrt both the primal and dual variables,
i.e. all of $x,u,v$.
}
...
as the simplest nonlinear optimization problem {\color{red}(speculation)}.
